...
if __name__ == "__main__":
    ...
    parser.add_argument('--show_output', '-o', default=False, action="store_true", help="Show output of the last results in terminal (max 25 results)")
    parser.add_argument('--db_file', '-db', default="crawl_results.db", help="Path to crawl results and blocklist database")
    parser.add_argument('--blocklists', '-bf', default=None, help="Path to custom blocklists file")
    ...
    if args.blocklists != None:
        ...
        if not any([args.blocklists.endswith('.yaml'), args.blocklists.endswith('.yml')]):
    ...
    blocklists = Blocklists(db_file=args.db_file, source_file=args.blocklists, force_bootstrap=args.bootstrap)

    results = asyncio.run(process_urls(
        urls=urls, 
        batch_size=args.batch_size, 
        tracking_domains_list=blocklists.get_domains(), 
